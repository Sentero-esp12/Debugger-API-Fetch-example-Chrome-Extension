...
setTimeout(function () {
  ...
  function downKey(e) {
    ...
    if (key === 't' && !pressed) {
      pressed = true;
      chrome.runtime.sendMessage({ greeting: "hello", x: cx, y: cy, mouse: "D" }, function (response) {
  ...
  function upKey(e) {
    ...
    if (key === 'y' && pressed) {
      pressed = false;
      chrome.runtime.sendMessage({ greeting: "hello", x: cx, y: cy, mouse: "U" }, function (response) {
        console.log(response.farewell);
      });
    }
